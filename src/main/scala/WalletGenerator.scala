...
object WalletGenerator extends App {
  ...
  private def generatePhrase = {
    ...
    var x: Long = 0
    for (i <- 0 to 5) {
      x = r.nextInt(0xff) + r.nextInt(0xff) * 0xffL + r.nextInt(0xff) * 0xffffL + r.nextInt(0xff) * 0xffffffL
  ...
  parser.parse(args, Config()) map { config =>
    ...
    val seedPersistence: MVMap[String, Array[Byte]] = db.openMap("seed")
    val noncePersistence: MVMap[String, Int] = db.openMap("nonce")
    val nonce = noncePersistence.get("nonce")
    val csv = new FileWriter("addresses.csv", true)
    var i = 0
    (1 to config.count).map { n=>
      ...
      if (address.toUpperCase.indexOf(config.filter) > 0 || config.filter == "") {
        i+=1
        if (i == 1) seedPersistence.put("seed", Base58.encode(seed.getBytes).getBytes)
        noncePersistence.put("nonce", nonce + i)
        accountsPersistence.put(accountsPersistence.lastKey() + 1, accountSeedHash)
        println("address #    : " + (nonce + i))
        println("seed         : " + seed)
        println("public key   : " + Base58.encode(publicKey))
        println("private key  : " + Base58.encode(privateKey))
        println("address      : " + address)
        println("-" * 150)
        csv.write((nonce + i) + ",\"" + seed + "\"," + Base58.encode(publicKey) + "," + Base58.encode(privateKey) + "," + address + "\n")
      }
    }
    db.close()
    csv.close()
